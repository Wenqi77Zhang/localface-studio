...
当前状态：阶段 0–15 的候选版工程与全面审查已完成，版本为 `1.0.0rc2`；审查发现的冻结需求差距、质量限制和人工验收项仍未清零，因此暂不创建稳定 `v1.0.0` 标签。
...
- [阶段 14 进展日志](docs/progress/phase-14.md)
- [阶段 15 全面审查日志](docs/progress/phase-15.md)
- [v1 最终集中验收清单](docs/FINAL_ACCEPTANCE.md)
- [候选版使用与验收演练](docs/ACCEPTANCE_WALKTHROUGH.md)
...
阶段 0–15 已形成可使用的本地单张照片候选产品、真实研究后端、质量基线、隐私安全验收、可选 ComfyUI 适配、Windows 交付链和公开仓库治理。阶段 15 使用真实 Edge 复核了上传、多脸选择、门禁提示、处理、导出、图库、下载和清除，并修复缺失站点图标导致的浏览器 404。全面审查同时确认：冻结策划要求的“基础结果与可选增强结果双工件对比”以及独立 `FaceSwapper` / `FaceEnhancer` 边界尚未实现；当前质量预设是每次任务二选一，不能冒充双结果。阶段 6 的 7 组真实样例中仍有 2 组明显伪影。视频仅冻结 v2 接口，尚未实现。公网生产仍被许可证、账户隔离、滥用治理和法律审查门禁阻止；所有研究权重、测试人物图片、评测明细与生成结果始终由 Git 隔离。
