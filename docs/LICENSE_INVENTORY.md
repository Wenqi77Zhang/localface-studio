...
| OpenCV / opencv-python-headless | YuNet 加载、图片矩阵与检测接口 | Apache-2.0 | 绿色 | 作为阶段 3 默认候选依赖；锁定前验证 Python 3.14 和 Windows 兼容性。 |
...
- OpenCV 官方许可证：https://github.com/opencv/opencv/blob/4.x/LICENSE
- OpenCV Zoo YuNet 说明与目录许可证：https://github.com/opencv/opencv_zoo/tree/main/models/face_detection_yunet
...
8. 检测器切换必须使旧检测 ID、检测框和人物选择失效，不能跨模型复用。
9. 自训练模型必须记录训练数据许可、训练配置、基础权重关系、评测结果和模型卡。
